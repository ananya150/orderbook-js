import { Order, OrderType, OrderUpdate, TimeInForce, Side } from "../src/order";
import { OrderBook } from "../src/book";
...
describe("OrderBook", () => {
  ...
  test("test limit", () => {
    ...
    expect(process3.err?.message).toBe("orderbook: order already exists");

    const process4 = ob.limit(Side.SELL, `fake-70`, 0, 40);
    expect(process4.err?.message).toBe("orderbook: invalid order quantity");

    // @ts-ignore
    const process5 = ob.limit("unsupported-side", `order-70`, 70, 100);
    expect(process5.err?.message).toBe(
      "orderbook: given neither 'bid' nor 'ask'"
    );
    ...
    expect(process7.err?.message).toBe("orderbook: invalid order quantity");
    ...
    expect(process8.err?.message).toBe("orderbook: invalid order quantity");
    ...
    expect(process9.err?.message).toBe("orderbook: invalid order price");

    // @ts-ignore
    const process10 = ob.limit(Side.SELL, `fake-wrong-price`, 10);
    expect(process10.err?.message).toBe("orderbook: invalid order price");

    // @ts-ignore
    const process11 = ob.limit(Side.SELL, `unsupported-tif`, 10, 10, "FAKE");
    expect(process11.err?.message).toBe(
  ...
  test("test limit FOK and IOC", () => {
    ...
    expect(process1.err?.message).toBe(
    ...
    expect(process2.err?.message).toBe(
      "orderbook: limit FOK order not fillable"
    );
    ...
    expect(process3.err?.message).toBe(
      "orderbook: limit FOK order not fillable"
    );
    ...
    expect(process4.err?.message).toBe(
      "orderbook: limit FOK order not fillable"
    );
  ...
  test("test market", () => {
    ...
    expect(process6.err?.message).toBe(
      "orderbook: given neither 'bid' nor 'ask'"
    );
  });
  ...
  test("test modify", () => {
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        expect(error?.message).toBe("orderbook: given neither 'bid' nor 'ask'");
      }
    }
