    ...
    function OrderBook() {
        ...
        this.limit = function (side, orderID, size, price, timeInForce) {
            ...
            if (quantityToTrade > 0) {
                var order = new Order(orderID, side, quantityToTrade, price, Date.now(), true);
                ...
                response.done.push(new Order(orderID, side, size, totalPrice_1 / totalQuantity_1, Date.now()));
            }
                ...
                while (orderQueue.len() > 0 && response.quantityLeft > 0) {
                    ...
                    if (headOrder) {
                        if (response.quantityLeft < headOrder.size) {
                            response.partial = new Order(headOrder.id, headOrder.side, headOrder.size - response.quantityLeft, headOrder.price, headOrder.time, true);
